...
public abstract class GenderArmorProvider implements DataProvider {
    ...
    private final Map<ResourceLocation, IGenderArmor> armorConfigs = new HashMap<>();
    ...
    public final CompletableFuture<?> run(@NotNull CachedOutput cachedOutput) {
        return this.registries.thenCompose(lookupProvider -> {
            ...
            for (Map.Entry<ResourceLocation, IGenderArmor> entry : armorConfigs.entrySet()) {
    ...
    protected void add(Holder<ArmorMaterial> armorMaterial, IGenderArmor armor) {
        add(armorMaterial.unwrapKey().map(ResourceKey::location)
              .orElseThrow(() -> new IllegalArgumentException("Unable to add a gender armor config for an unregistered item")), armor);
    }

    protected void add(ResourceLocation material, IGenderArmor armor) {
