...
public enum Gender implements StringRepresentable {
    ...
    FEMALE("female", WildfireLang.LABEL_FEMALE.translateColored(TextColor.LIGHT_PURPLE), true, WildfireSounds.INSTANCE::femaleHurt),
    MALE("male", WildfireLang.LABEL_MALE.translateColored(TextColor.BLUE), false, null),
    OTHER("other", WildfireLang.LABEL_OTHER.translateColored(TextColor.GREEN), true, WildfireSounds.INSTANCE::femaleHurt);
    //~}
    ...
    private final @Nullable Supplier<SoundEvent> hurtSound;

    Gender(String saveName, Component name, boolean canHaveBreasts, @Nullable Supplier<SoundEvent> hurtSound) {
    ...
    public @Nullable SoundEvent getHurtSound() {
        return hurtSound == null ? null : hurtSound.get();
    }
